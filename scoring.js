// Shared scoring engine — included on every page that needs scores
...
function finalScore(b, wRep, wSearch, wSite) {
  ...
  if (total === 0) return 0;
  return (wRep * reputationScore(b) + wSearch * searchScore(b) + wSite * siteScore(b)) / total;
}
...
function scoreAll(businesses, wRep, wSearch, wSite) {
  return businesses.map(b => ({
    ...
    score:       finalScore(b, wRep, wSearch, wSite),
  }));
}
